...
jQuery(document).ready(function ($) {
  // Repair attachments button handler
  $("#slm-repair-attachments").on("click", function () {
    var $button = $(this);
    var $status = $("#slm-repair-status");

    // Confirm action
    if (
      !confirm("This will attempt to repair damaged file references. Continue?")
    ) {
      return;
    }
    ...
    $status
      ...
      .html('<span class="slm-loading"></span> Repairing attachments...');
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          $status.addClass("error").html("✗ Error repairing attachments");
        }
      },
      error: function () {
  ...
  $("#slm-recalculate").on("click", function () {
    ...
    $status
      .removeClass("success error")
      .html('<span class="slm-loading"></span> Recalculating...');
  ...
  function updateUsageDisplay(totalBytes, formattedSize) {
    ...
    var maxStorageMB =
      parseInt($('input[name*="max_storage_mb"]').val()) || 1000;
    ...
    $(".slm-progress-bar").each(function () {
      ...
      // Update color class
      $bar.removeClass(
        "slm-progress-normal slm-progress-warning slm-progress-critical"
      );
      if (percentage >= 90) {
  ...
  function showInputError($input, message) {
    ...
    if ($existing.length) {
      ...
      $container.append(
        '<p class="slm-input-message error">' + message + "</p>"
      );
    }
  }
  ...
  function showInputWarning($input, message) {
    ...
    if ($existing.length) {
      ...
      $container.append(
        '<p class="slm-input-message warning">' + message + "</p>"
      );
    }
  }
  ...
  $(".slm-progress-container").each(function () {
    ...
    $container.on("mouseenter", function () {
      ...
      var percentage =
        (parseFloat(width) / parseFloat($container.width())) * 100;
  ...
  // Monitor for automatic recalculation notifications
  function checkForAutoRecalculation() {
    $.ajax({
      url: slm_ajax.ajax_url,
      type: "POST",
      data: {
      ...
      success: function (response) {
        if (response.success && response.data.auto_recalculated) {
          showAutoRecalculationNotice();
          // Update display with new data
          updateUsageDisplay(
            response.data.total_bytes,
            response.data.formatted.total
          );
        }
      },
      error: function () {
  ...
  // Show notification when automatic recalculation occurs
  function showAutoRecalculationNotice() {
    var $notice = $(
      '<div class="notice notice-info is-dismissible slm-auto-recalc-notice">' +
        "<p><strong>Storage Limit Manager:</strong> Usage automatically recalculated after file upload.</p>" +
  ...
  // Check for auto-recalculation every 30 seconds on relevant pages
  if (
    window.location.href.indexOf("upload.php") !== -1 ||
    window.location.href.indexOf("media-new.php") !== -1 ||
    window.location.href.indexOf("storage-limit-manager") !== -1
  ) {
    setInterval(checkForAutoRecalculation, 30000); // 30 seconds
  }
